...
total_boxes, points = detect_face(image, 20, pnet, rnet, onet, [0.5, 0.8, 0.9], 0.709)
t2 = time.time()
print("detect time: %.2fms" %((t2-t1)*1000))
print('box')
...
for idx, (bounding_box, keypoints) in enumerate(zip(total_boxes, points.T)):
        ...
        if crop_image is not None:
                ...
                cv2.putText(org_image, person[9:], (bounding_box[0], bounding_box[1]-2), cv2.FONT_HERSHEY_SIMPLEX,
                                                                        1., (0, 0, 255), 3, lineType=cv2.LINE_AA)
                t2 = time.time()
                print("recognize time: %.2fms" %((t2-t1)*1000))

cv2.imwrite('./result/hh.jpg', org_image)
